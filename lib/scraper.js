...
async function textpro(url, text) {
  ...
  let caritoken = await geturl.text();
  let setCookieHeader = geturl.headers.get("set-cookie");
  let hasilcookieParsed = setCookieHeader
    ? setCookieHeader
    ...
    : {};
  let hasilcookie = {
    __cfduid: hasilcookieParsed.__cfduid,
    PHPSESSID: hasilcookieParsed.PHPSESSID,
  };
...
text.forEach((texts) => form.append("text[]", texts));
...
module.exports = {instagram4, capcutdl, instagram3, mediafiredll, cekkuota,tele, instagram2, ytPlayMp4, ytPlayMp3, kodepos, pinterest, igdl, textpro, mediafire, ffstalk, mlstalk, Tiktok,surah,getSurah,listsurah, tafsirsurah, ephoto, emoji}
